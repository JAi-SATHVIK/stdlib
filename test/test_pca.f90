...
contains
    ...
    subroutine test_pca_sp(error)
        ...
        real(sp) :: x(3, 2), x_copy(3, 2), components(2, 2), s(2), mu(2)
        ...
        call check(error, size(x_trans, 1) == 3 .and. size(x_trans, 2) == 2, "pca_sp transform_no_mean shape")
        if (allocated(error)) return
        ...
        call check(error, err%ok(), "pca_sp cov err")
        if (allocated(error)) return

        call check(error, abs(s(1) - 4.0_sp) < sptol, "pca_sp cov s1")
        if (allocated(error)) return

        ! Test overwrite_x=.true.
        x_copy = x
        call pca(x_copy, components, s, method="svd", overwrite_x=.true., err=err)
        call check(error, err%ok(), "pca_sp overwrite err")
        if (allocated(error)) return

        call check(error, abs(s(1) - 4.0_sp) < sptol, "pca_sp overwrite s1")
        if (allocated(error)) return

        ! Test case-insensitivity
        call pca(x, components, s, method="SVD", err=err)
        call check(error, err%ok(), "pca_sp case_insensitive err")
        if (allocated(error)) return

    end subroutine test_pca_sp
    subroutine test_pca_dp(error)
        type(error_type), allocatable, intent(out) :: error
        real(dp) :: x(3, 2), x_copy(3, 2), components(2, 2), s(2), mu(2)
        ...
        ! Test Transform without x_mean (keyword-based call)
        call pca_transform(x, components, x_transformed=x_trans)
        call check(error, size(x_trans, 1) == 3 .and. size(x_trans, 2) == 2, "pca_dp transform_no_mean shape")
        if (allocated(error)) return
        ...
        ! Test COV method (alias for EIG)
        call pca(x, components, s, method="cov", err=err)
        call check(error, err%ok(), "pca_dp cov err")
        if (allocated(error)) return

        call check(error, abs(s(1) - 4.0_dp) < dptol, "pca_dp cov s1")
        if (allocated(error)) return

        ! Test overwrite_x=.true.
        x_copy = x
        call pca(x_copy, components, s, method="svd", overwrite_x=.true., err=err)
        call check(error, err%ok(), "pca_dp overwrite err")
        if (allocated(error)) return

        call check(error, abs(s(1) - 4.0_dp) < dptol, "pca_dp overwrite s1")
        if (allocated(error)) return

        ! Test case-insensitivity
        call pca(x, components, s, method="SVD", err=err)
        call check(error, err%ok(), "pca_dp case_insensitive err")
        if (allocated(error)) return
