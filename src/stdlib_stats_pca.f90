...
contains
  ...
  ! Helper subroutine: Centers data in-place by subtracting the mean from each column
    subroutine center_data_sp(x, mu)
  ...
  ! SVD-based PCA driver: computes principal components via SVD of centered data
    subroutine pca_svd_driver_sp(x_centered, n, p, components, singular_values, err)
      use stdlib_blas_constants_sp, only: zero
      real(sp), intent(inout) :: x_centered(:,:)
    ...
    end subroutine pca_svd_driver_sp
    subroutine pca_svd_driver_dp(x_centered, n, p, components, singular_values, err)
      use stdlib_blas_constants_dp, only: zero
      real(dp), intent(inout) :: x_centered(:,:)
      integer(ilp), intent(in) :: n, p
    ...
    end subroutine pca_svd_driver_dp

  ! Eigendecomposition-based PCA driver: computes principal components via covariance matrix
    subroutine pca_eigh_driver_sp(x_centered, n, p, components, singular_values, err)
      use stdlib_blas_constants_sp, only: zero
      real(sp), intent(in) :: x_centered(:,:)
      integer(ilp), intent(in) :: n, p
      real(sp), intent(out) :: components(:,:)
      real(sp), intent(out) :: singular_values(:)
      type(linalg_state_type), intent(out) :: err
      ...
      alpha = 1.0_sp / real(max(n-1, 1), sp)
      beta = zero
    ...
    subroutine pca_eigh_driver_dp(x_centered, n, p, components, singular_values, err)
      ...
      ! Compute covariance matrix using BLAS syrk: C = (1/(n-1)) * X^T * X
      alpha = 1.0_dp / real(max(n-1, 1), dp)
      beta = zero
      allocate(c(p, p), source=zero)
      call syrk('U', 'T', p, n, alpha, x_centered, n, beta, c, p)
    ...
    module subroutine pca_transform_sp(x, components, x_mean, x_transformed)
      use stdlib_blas_constants_sp, only: one, zero
      real(sp), intent(in) :: x(:,:)
    ...
    module subroutine pca_transform_dp(x, components, x_mean, x_transformed)
      use stdlib_blas_constants_dp, only: one, zero
      real(dp), intent(in) :: x(:,:)
